...
from meitner.httpclient import HttpClient
from meitner.sdkconfiguration import SDKConfiguration
...
class ClientCredentialsHook(SDKInitHook, BeforeRequestHook, AfterErrorHook):
    ...
    def get_credentials_global(self, security: Any) -> Optional[Credentials]:
        if security is None or security.option2 is None:
            return None

        # Extract additional properties from security object
        additional_properties = {}
        for key, value in dict(security.option2).items():
            if key not in ["client_id", "client_secret", "token_url", "scopes"]:
                additional_properties[key] = value

        return Credentials(
            client_id=security.option2.client_id,
            client_secret=security.option2.client_secret,
            token_url=security.option2.token_url,
            scopes=None,
